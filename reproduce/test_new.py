...
# train
X_pos_train = import_data("./dataset/sequences/%s_pos.fa" % TRAIN_SPECIES)
X_neg_train = import_data("./dataset/sequences/%s_neg.fa" % TRAIN_SPECIES)
...
# test
X_pos_test = import_data("./dataset/sequences/%s_pos.fa" % TEST_SPECIES)
X_neg_test = import_data("./dataset/sequences/%s_neg.fa" % TEST_SPECIES)
...
#model.load_weights("./weights/test_new/%s_test.hdf5" % TEST_SPECIES)

for i in range(N_EPOCH):
 print("fold:",str(1),"epoch:",str(i+1))
 history = model.fit(X_train, Y_train, epochs=1, verbose=1, batch_size = 256, class_weight='auto')
 predictions = model.predict(X_test,verbose=0)
 rst = perfeval(predictions, Y_test, verbose=1)
 wrtrst(WriteFile,rst,0,i)

model.save_weights(filepath="./weights/test_new/%s_test.hdf5" % TEST_SPECIES, overwrite=True)
WriteFile.close()
