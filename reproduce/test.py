...
for _species in SPECIES:
 ...
 # train
 X_pos_train = import_data("./dataset/cv/%s/%s_pos_all.fa" % (_species,_species))
 X_neg_train = import_data("./dataset/cv/%s/%s_neg_all.fa" % (_species,_species))
 ...
 model.load_weights("./weights/test/%s_test.hdf5" % _species)
 ...
 model.save_weights(filepath="./weights/test/%s_test.hdf5" % _species, overwrite=True)
 WriteFile.close()
